...
class FinancialSnapshot:
    ...
    def __post_init__(self):
        ...
        if self.shares_outstanding <= 0:
            raise ValueError(f"Shares outstanding must be positive: {self.shares_outstanding}")
    ...
    def return_on_invested_capital(self) -> Decimal:
        ...
        return (after_tax_ebit / self.invested_capital) if self.invested_capital > 0 else Decimal('0')
...
class ComparableCompany:
    ...
    def from_financial_snapshot(
        ...
        # Calculate multiples
        ev_revenue = snapshot.enterprise_value / snapshot.revenue if snapshot.revenue > 0 else Decimal('0')
        ev_ebitda = snapshot.enterprise_value / snapshot.ebitda if snapshot.ebitda > 0 else Decimal('0')
        pe_ratio = snapshot.market_cap / snapshot.net_income if snapshot.net_income > 0 else Decimal('0')
        
        # Calculate growth rate
        revenue_growth = (snapshot.revenue - prior_year_revenue) / prior_year_revenue if prior_year_revenue > 0 else Decimal('0')
...
class ComparableAnalysis:
    ...
    def robust_medians(self) -> Dict[str, Optional[Decimal]]:
        ...
        return {
            ...
            "ev_ebitda": Decimal(str(self._robust_center(ev_ebitda))) if ev_ebitda else None,
    ...
    def _regress_multiple_on_drivers(
        self, comps: List[ComparableCompany], selector: Callable[[ComparableCompany], float]
    ...
    def regression_adjusted_multiples_for_target(
        ...
        out: Dict[str, Optional[Decimal]] = {"ev_rev": None, "ev_ebitda": None, "pe": None}

        # EV/Revenue regression
        rr = self._regress_multiple_on_drivers(self._valid_ev_rev(), lambda c: float(c.ev_revenue))
        if rr is not None:
            beta, _ = rr
        ...
        re = self._regress_multiple_on_drivers(self._valid_ev_ebitda(), lambda c: float(c.ev_ebitda))
        if re is not None:
            beta, _ = re
            pred = sum(beta[i]*X_target[i] for i in range(len(beta)))
            if pred > 0:
        ...
        # P/E regression
        rp = self._regress_multiple_on_drivers(self._valid_pe(), lambda c: float(c.pe_ratio))
        if rp is not None:
            beta, _ = rp
            pred = sum(beta[i]*X_target[i] for i in range(len(beta)))
            if pred > 0:
...
def value_using_comps(
    ...
    if use_regression:
        reg = analysis.regression_adjusted_multiples_for_target(t_growth, t_margin, t_roic)
    ...
    if multiples.get("pe") and snapshot.net_income > 0 and snapshot.shares_outstanding > 0:
        price_per_share = multiples["pe"] * (snapshot.net_income / snapshot.shares_outstanding)
...
def value_target_with_both_methods(profile: CompanyValuationProfile) -> Dict[str, object]:
    """
